...
bool CConfig::BuildConfig(CMainLoop& mainloop, std::vector<CDevice*>& devices, std::vector<CLight>& lights)
{
  ...
  //Build DeviceConfig
  if (!BuildDeviceConfig(devices, mainloop))
    return false;
...
void CConfig::BuildServerConfig(CMainLoop& mainloop)
{
  ...
  mainloop.SetInterface(interface, port);
}
...
bool CConfig::BuildDeviceConfig(std::vector<CDevice*>& devices, CMainLoop& mainloop)
{
  for (int i = 0; i < m_devicelines.size(); i++)
  {
    string line;
    ...
    {
      CDevice* device = NULL;
      if (!BuildPopen(device, i, mainloop))
      {
        if (device)
          delete device;
        return false;
    ...
    {
      CDevice* device = NULL;
      if (!BuildRS232(device, i, mainloop, type))
      {
        if (device)
          delete device;
        return false;
    ...
    {
      CDevice* device = NULL;
      if (!BuildLtbl(device, i, mainloop))
      {
        if (device)
          delete device;
        return false;
    ...
    {
      CDevice* device = NULL;
      if (!BuildDioder(device, i, mainloop))
      {
        if (device)
          delete device;
        return false;
    ...
    {
      CDevice* device = NULL;
      if (!BuildAmbioder(device, i, mainloop))
      {
        if (device)
          delete device;
        return false;
    ...
    {
      CDevice* device = NULL;
      if (!BuildiBeLight(device, i, mainloop))
      {
        if (device)
          delete device;
        return false;
    ...
    {
      CDevice* device = NULL;
      if (!BuildLightpack(device, i, mainloop))
      {
        if (device)
          delete device;
        return false;
...
bool CConfig::BuildPopen(CDevice*& device, int devicenr, CMainLoop& mainloop)
{
  device = new CDevicePopen(mainloop);
...
bool CConfig::BuildRS232(CDevice*& device, int devicenr, CMainLoop& mainloop, const std::string& type)
{
  CDeviceRS232* rs232device = new CDeviceRS232(mainloop);
  device = rs232device;
...
bool CConfig::BuildLtbl(CDevice*& device, int devicenr, CMainLoop& mainloop)
{
  device = new CDeviceLtbl(mainloop);
...
bool CConfig::BuildiBeLight(CDevice*& device, int devicenr, CMainLoop& mainloop)
{
  CDeviceiBeLight* ibedevice = new CDeviceiBeLight(mainloop);
  device = ibedevice;
  ...
  SetDeviceSerial(ibedevice, devicenr);
  SetDeviceAllowSync(device, devicenr);
  SetDeviceDebug(device, devicenr);
  SetDeviceThreadPriority(device, devicenr);
...
bool CConfig::BuildLightpack(CDevice*& device, int devicenr, CMainLoop& mainloop)
{
  CDeviceLightpack* lightpackdevice = new CDeviceLightpack(mainloop);
...
bool CConfig::BuildDioder(CDevice*& device, int devicenr, CMainLoop& mainloop)
{
  CDeviceDioder* dioderdevice = new CDeviceDioder(mainloop);
...
bool CConfig::BuildAmbioder(CDevice*& device, int devicenr, CMainLoop& mainloop)
{
  CDeviceAmbioder* ambioderdevice = new CDeviceAmbioder(mainloop);
...
{
  string posline, nameline, namevalue, posvalue;
  int linenr = GetLineWithKey("name", lines, nameline);
  if (linenr == -1)
  {
  ...
  linenr = GetLineWithKey("position", lines, posline);
  if (linenr == -1)
  {
    LogError("%s: light %i has no position", m_filename.c_str(), lightnr + 1);
    return false;
  }
  GetWord(nameline, namevalue);
  GetWord(posline, posvalue);

  light.SetName(namevalue);
  light.SetPosition(posvalue);
