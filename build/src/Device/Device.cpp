...
#include "Util/fastapprox/fastpow.h"
...
float CChannel::GetValue(int64_t time)
{
  ...
  else
  {
    ...
    float speed = fastpow(2.0, (m_speed - 100.0) / 15.0); //exponential speed, makes the value a lot saner
                                                      //the value is halved for every 15 counts that speed lowers by
    float speedadjust = 1.0 - fastpow(1.0 - speed, timediff / 20000.0); //speed adjustment value, corrected for time
  ...
  //gamma correction
  if (m_gamma != 1.0)
    outputvalue = fastpow(outputvalue, m_gamma);
...
CDevice::CDevice(CMainLoop& mainloop) : m_mainloop(mainloop)
{
  m_type = NOTHING;
  m_allowsync = true;
  m_debug = false;
...
void CDevice::Process()
{
  ...
  while(!m_stop)
  {
    ...
    while(!m_stop)
    {
      ...
      {
        CloseDevice();
        LogError("%s: setting up failed...", m_name.c_str());
    ...
    while(!m_stop)
    {
      if (!WriteOutput())
      {
        ...
        Log("%s: io operation failed...", m_name.c_str());
        Log("Exiting EnigmaLight...");
        exit(1);
